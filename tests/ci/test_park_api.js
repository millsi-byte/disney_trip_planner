...
(async () => {
  ...
  const out = await page.evaluate(async () => {
    ...
    // ── Build 410: everyone-mode to-dos (each person checks off their own) ──
    const mem410 = tripMembers().filter(p => p !== 'scott');
    TODO.push({ id: 'tde1', trip: 'jul26', by: 'scott', n: 'ZZ Online check-in', when: '', who: [mem410[0]], priv: false, each: true, done: false, doneBy: {} });
    S.persona = 'scott';
    tdToggle('tde1');
    ...
    S.persona = mem410[0];
    r.eachRowOtherUnchecked = todoRow(tde1, false).indexOf('chkbox on') < 0;
    tdToggle('tde1');
    r.eachAllDone = !!tde1.doneBy['scott'] && !!tde1.doneBy[mem410[0]];
    S.persona = 'scott';
