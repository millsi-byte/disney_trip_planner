...
var DATA_VERSION='11';
var BUILD='242';   /* bumped each deploy — shown in Settings to spot stale caches */
...
function scrTicketEdit(){
  ...
  if(S._tkCat==='date'){
    ...
    body+='</div>';
    body+='<div class="field"><div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px"><label class="field-label" style="margin-bottom:0">Activation date</label><span id="tk-exp" style="font-size:13px;color:var(--muted)">'+esc(expDisplay(S._tkAct)||'—')+'</span></div><input type="date" class="field-input" id="tk-activation" value="'+esc(S._tkAct||'')+'" onchange="tkSyncExp()"></div>';
