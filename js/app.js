...
var DATA_VERSION='11';
var BUILD='294';   /* bumped each deploy — shown in Settings to spot stale caches */
...
function flightCard(flts,d){
  ...
  if(S.open[key]){
    ...
    for(var i=0;i<flts.length;i++){
      if(i>0) o+='<div class="flt-sep"><span class="flt-sep-pill">'+IC.planexs+'</span></div>';
      o+='<div class="flt-jbox'+(isPlanningStatus(flts[i].status)?' planning':'')+'">'+flightJourney(flts[i],d)+'</div>';
    }
    o+='</div>';
  }
  o+='</div>';
